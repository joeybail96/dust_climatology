...
def _get_tdump_files(base_dir):
    ...
    tdump_files = []
    for period in range(1, 31):  # Iterate over periods from 1_period to 30_period
...
def generate_hysplit_array(working_dir, num_particles=27):
    ...
    all_trajectory_data = []  # List to store trajectory data for all particles in all tdump files

    # Debugging: Print the number of tdump files
    ...
    # Iterate through all tdump files
    for tdump_file in tdump_files:
        ...
        for particle_id in range(num_particles):  # Assuming num_particles is set to 10 for particles 0-9
            z = z+1    
            trajectory_data = _read_tdump(tdump_file, particle_id)
            
            if trajectory_data:
                timestamp, locations = trajectory_data
                timestamp = np.datetime64(timestamp)
                
                if len(locations) != 25:
                    print(f"Error: Particle {particle_id} in {tdump_file} has {len(locations)} locations, expected 25.")
                    x = x + (25-len(locations))
                    xxx = xxx+1
                    for location_placehold in range(1, 26):  # This creates a loop from 1 to 25 (inclusive)
                        all_trajectory_data.append([timestamp, particle_id, np.nan, np.nan, np.nan])
                        zzz = zzz+1
                    continue  # Skip this particle if the number of locations is not 25
                zz = zz+1
                

                # Debugging: Print the number of locations per particle
                print(f"Particle {particle_id} has {len(locations)} locations.")
    ...
    # Debugging: Check the size of the trajectory array and unique time steps
    print(f"Generated hysplit_data with {len(trajectory_array)} rows.")
    print(f"Number of unique time steps: {len(np.unique(trajectory_array[:, 0]))}")
...
def generate_hysplit_ds(grimm_ds, hysplit_array, hysplit_file, merged_file):
    ...
    # Convert hysplit_array to an xarray Dataset
    times = np.unique(hysplit_array[:, 0])  # Unique timestamps
    particles = np.arange(27)  # 27 particles
    locations = np.arange(25)  # 25 locations per particle

    # Create empty arrays to store the reshaped data
    ...
    # Fill arrays with HYSPLIT data
    for i, time in enumerate(times):
        for j, particle in enumerate(particles):
            mask = (hysplit_array[:, 0] == time) & (hysplit_array[:, 1] == particle)
            subset = hysplit_array[mask]
            
            if len(subset) == 25:  # Ensure it matches the expected structure
                lons[i, j, :] = subset[:, 2]  # Longitude
                lats[i, j, :] = subset[:, 3]  # Latitude
                alts[i, j, :] = subset[:, 4]  # Altitude
